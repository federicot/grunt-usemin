  ...
  lines.forEach(function (l) {
    ...
    if (block && last) {
      ...
      if (asset && asset[2]) {
        ...
        var data = l.match(/data-([^'"]+)=['"]([^'"]+)['"]/g);
        if (data) {
          data.forEach(function(d) {
            var attr = d.match(/data-([^'"]+)=['"]([^'"]+)['"]/);
            ...
            if (attr[1] === 'main') {
              ...
              last.requirejs.baseUrl = path.join(originDir, path.dirname(attr[2]));
              last.requirejs.name = path.basename(attr[2]);
              ...
              // detect duplicates
              last.data.forEach(function(exist) {
                if (attr[1] === exist.name) {
                  throw 'Error: Duplicate data attribute "' + attr[1] + '" found in block.';
                }
              });
...
HTMLProcessor.prototype.replaceWith = function replaceWith(block) {
  ...
  } else if (block.data !== undefined || block.requirejs !== undefined) {
    var attr = [];

    if (block.requirejs !== undefined) {
      ...
      attr.push('data-main="' + dataMain + '"');
    }

    if (block.data !== undefined) {
      block.data.forEach(function(d) {
        attr.push('data-' + d.name + '="' + d.value + '"');
      });
    }

    result = block.indent + '<script ' + attr.join(' ') + ' src="' + dest + '"><\/script>';
  } else if (block.defer) {
